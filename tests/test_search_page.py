...
from src.similarity_search import SIMILARITY_METRICS_BY_KEY
...
def test_result_card_omits_empty_metadata_sections() -> None:
    ...
    html = result_card_html(1, row, SIMILARITY_METRICS_BY_KEY["cosine"])
...
def test_result_card_score_details_show_snowflake_similarity() -> None:
    ...
    metric = SIMILARITY_METRICS_BY_KEY["cosine"]
...
def test_unbounded_metric_omits_score_bar() -> None:
    metric = SIMILARITY_METRICS_BY_KEY["l2"]
    row = pd.Series(
        {
            "claim_id": "1",
            "document": "Claim description: Pipe leak in building.",
