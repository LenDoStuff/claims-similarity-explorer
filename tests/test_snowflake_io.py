...
from src.snowflake_io import add_embedding_columns, initialized_models, prepare_claims_frame
...
class FakeDataFrame:
    def __init__(self, schema_names: list[str], calls: dict) -> None:
...
def test_add_embedding_columns_supports_multiple_models() -> None:
    calls = {}
    frame = FakeDataFrame(["DOCUMENT"], calls)

    add_embedding_columns(frame, ["voyage-multilingual-2", "e5-base-v2"])
...
def test_add_embedding_columns_requires_a_model() -> None:
    with pytest.raises(ValueError, match="at least one"):
        add_embedding_columns(FakeDataFrame(["DOCUMENT"], {}), [])
